...
try {
    ...
    const getBodyStr = (sysPrompt, userPrompt) => {
        // We must pass actual object structure instead of a stringified object into n8n's expression engine
        // to prevent deep syntax parsing errors with nested template literals.
        // By keeping it as a raw JS expression inside the {{ }}, n8n evaluates it cleanly.

        // Notice how we use backticks for the strings, and properly escape the inner n8n syntax
        return `={{ JSON.stringify({
      model: 'llama-3.3-70b-versatile',
      messages: [
        { role: 'system', content: \`${sysPrompt.replace(/`/g, '\\`')}\` },
